...
// Id 컴포넌트 (자식)
// Pw 컴포넌트 (자식)
// Exam 3 컴포넌트 (부모) - 해당 파일에서 내보낼 기본 컴포넌트

// Id 컴포넌트 (자식)
const Id = (props) => { // 부모에게서 전달받은 속성을 props라고 부름    props : {onChangeId, name, age...}
...
// Pw 컴포넌트 (자식)
const Pw = ({onChangePw}) => { // {}를 이용하여 props 안에 있는 값을 바로 꺼내서 쓸 수도 있다
...
const Exam3 = () => {
    ...
    return (
        ...
        <>
            {/* 컴포넌트 중 Id를 불러 랜더링함 (Id가 Exam3의 자식이 됨) */}
            <Id onChangeId={onChangeId}/>
                {/* Key Value 형태로 넘어감 */}
            
            {/* 컴포넌트 중 Pw를 불러 랜더링함 (Pw가 Exam3의 자식이 됨) */}
