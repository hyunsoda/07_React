...
2. state(상태) : 컴포넌트의 내부에서 관리되며 컴포넌트의 동작 및 UI 랜더링을 제어하는데 사용 
...
class Exam1 extends Component {
  ...
  /*
        ...
        - 상태(state)는 React 컴포넌트에서 데이터가 저장되는 곳으로 , *상태값이 변경될 때 마다 컴포넌트가 다시 랜더링된다. *
        - 변수는 컴포넌트 내에서 상태가 아닌 다른 데이터를 저장하기 위해 사용되므로 변수값이 변경되어도 컴포넌트는 다시 랜더링되지 않는다.
  ...
  handleClick = () => {
    ...
    // exam1이라는 컴포넌의 상태 (state)중 count를 현재 상태값에서 +1 한 값으로 상태를 다시 세팅 (변경).
  };

  // render 함수 (클래스형 컴포넌트에서 필수 구문)
  render() {
    // 화면에 띄워줄 구문을 render함수 안에 작성
...
// 해당 Exam1.js 파일에서 기본적으로 내보내는 컴포넌트의 이름은 Exam1임.
